...
# walk +Z across z=128 (region row boundary), polling region state every ~3s
...
while time.time() - t0 < 90:
    ...
    if now - last_poll >= 3:
        last_poll = now
        try:
            out = rcon.cmd("folia regions")
            # keep just the region that owns the player's chunk
            import re
            cx, cz = int(p[0]) // 16, int(p[2]) // 16
            m = re.search(rf"minecraft:overworld:{cx}\s*,[^\n]*\n[^\n]*", out)
            line = m.group(0).replace("\n", " | ") if m else f"chunk {cx},{cz} not listed"
            if line not in regions_seen:
                regions_seen.append(line)
            print(f"[X] t={now - t0:5.1f}s z={p[2]:7.2f} chunk=({cx},{cz}) {line}", flush=True)
        except Exception as e:
            print("[X] rcon poll failed:", e, flush=True)
    if p[2] >= target_z:
        print("[X] crossed boundary (target reached)", flush=True)
        break

print("[X] walk: dist", bot.pos[2] - sz, "tps", getattr(bot, "_tp_count", 0) - tp0, flush=True)

# place/break at the boundary column
...
bot.play_loop(1.5)
print("[X] placed", bot.blocks_placed, "broke", bot.blocks_broken, flush=True)
...
# region state after respawn
try:
    out = rcon.cmd("folia regions")
    print("[X] final region snapshot (overworld rows):")
